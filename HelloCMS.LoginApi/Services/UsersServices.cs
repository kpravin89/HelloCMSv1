...
namespace HelloCMS.Identity.Services
{
    public class UsersServices : IUsersServices
    {
        ...
        public async Task<SelectUserVM?> RegisterAsync(RegisterUserVM registerVM)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return _mapper.Map<SelectUserVM>(createdUser);
            }
...
{result.Errors.Select(a => a.Description).Aggregate((a, b) => a + "\r\n" + b)}";
        ...
        public async Task<SelectUserVM?> GetUser(int Id)
        {
            var userByUserId = await GetById(Id);
            return _mapper.Map<SelectUserVM>(userByUserId);
        }

        public async Task<SelectUserVM?> GetUser(string usernameOrEmail)
        {
            //Find user by User Name
            var usersByUserName = await _userManager.FindByNameAsync(usernameOrEmail);
            if (usersByUserName != null)
                return _mapper.Map<SelectUserVM>(usersByUserName);

            //Find user by User Primary Email ID.
            var usersByEmail = await _userManager.FindByEmailAsync(usernameOrEmail);
            if (usersByEmail != null)
                return _mapper.Map<SelectUserVM>(usersByEmail);

            return null;
        }

        public async Task<List<SelectUserVM>?> GetUsers(string? Roles)
        {
            ...
            if (RolesList != null)
            {
                ...
                var Result = new List<SelectUserVM>();
                ...
                RolesList.ForEach(async Role =>
                {
                    ...
                    if (users != null)
                    {
                        var selectUserVMList = users.Select(user => _mapper.Map<SelectUserVM>(user)).ToList();
            ...
            else
            {
                ...
                if (users != null)
                    return users.Select(user => _mapper.Map<SelectUserVM>(user)).ToList();
                else
                    return null;
        ...
        public async Task Update(int Id, UpdateUserVM updateVM)
        {
            ...
            if (!result.Succeeded)
                throw new InvalidOperationException("User update failed with reason\r\n" + result.Errors.Select(a => $"Error Code: {a.Code} Description: {a.Description}").Aggregate((a, b) => a + "\r\n" + b));
        }
